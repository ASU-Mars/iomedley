...
iom_uncompress(FILE * fp, char *fname)
{
    ...
    // To fix the warning generated by using tempnam,
    // you'd want to create the temporary file programmatically
    // using mkstemp
    // and pipe the compress process to it.
    // then close the file ourselves.
    // As currently implemented, it's a *slight* security risk
    // and generates an annoying compile time warning
